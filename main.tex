...
\paragraph{V2 threshold and horizon sensitivity.}
The strongest objection is that the half threshold and 60-day window are
arbitrary. Table~\ref{tab:threshold-sensitivity} therefore reports a stricter
80\% threshold and a 120-day horizon. The sentinel row in
Table~\ref{tab:half_life} is censored at 61 days; it is not an infinite
half-life. When the horizon is extended to 120 days, sentinel-triggered rehearsal
crosses the half-success threshold at 89.0 days, while the oracle remains
censored. The 80\% threshold also exposes early degradation: frozen skills cross
at 7.4 days and calendar recalibration at 6.3 days despite higher day-60
success. The scalar half-life is therefore useful only with its threshold,
window, censoring flag, and preferably the full curve.

\input{docs/threshold_sensitivity_stress_table.tex}
...
\section{Limitations}
The simulation is intentionally small and does not establish a universal decay law. Real robot skill curves can be nonmonotonic, task-dependent, and expensive to estimate. The half threshold is also a convention rather than a law. The v2 stress shows that rankings and apparent non-crossing can change with the threshold and observation window. These limitations do not remove the measurement need. They motivate reporting the full decay curve, the threshold, the observation window, and the censoring rule alongside the scalar half-life.
